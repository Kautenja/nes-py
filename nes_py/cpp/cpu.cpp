...
void CPU::step(MainBus &m_bus) {
    ...
    NES_Byte opcode = m_bus.read(r_PC++);
...
bool CPU::executeImplied(MainBus &m_bus, NES_Byte opcode) {
    switch (static_cast<OperationImplied>(opcode)) {
        ...
        case JSR:
            ...
            pushStack(m_bus, static_cast<NES_Byte>((r_PC + 1) >> 8));
            pushStack(m_bus, static_cast<NES_Byte>(r_PC + 1));
        ...
        case JMP:
            ...
            break;
        case JMPI: {
                NES_Address location = readAddress(m_bus, r_PC);
...
bool CPU::executeBranch(MainBus &m_bus, NES_Byte opcode) {
    if ((opcode & BRANCH_INSTRUCTION_MASK) == BRANCH_INSTRUCTION_MASK_RESULT) {
        ...
        if (branch) {
            ...
            ++m_skipCycles;
            auto newPC = static_cast<NES_Address>(r_PC + offset);
    ...
    if ((opcode & INSTRUCTION_MODE_MASK) == 0x1) {
        ...
        switch (static_cast<AddrMode1>((opcode & ADRESS_MODE_MASK) >> ADDRESS_MODE_SHIFT)) {
            case IndexedIndirectX: {
                    NES_Byte zero_addr = r_X + m_bus.read(r_PC++);
            ...
            case Absolute:
                ...
                break;
            case IndirectY: {
                    NES_Byte zero_addr = m_bus.read(r_PC++);
                    location = m_bus.read(zero_addr & 0xff) | m_bus.read((zero_addr + 1) & 0xff) << 8;
                    if (op != STA)
        ...
        switch (op) {
            ...
            case ADC: {
                    ...
                    NES_Address sum = r_A + operand + f_C;
            ...
            case SBC: {
                ...
                break;
            case CMP: {
                    NES_Address diff = r_A - m_bus.read(location);
                    f_C = !(diff & 0x100);
                    setZN(diff);
                }
                break;
    ...
    if ((opcode & INSTRUCTION_MODE_MASK) == 0x0) {
        ...
        switch (static_cast<Operation0>((opcode & OPERATION_MASK) >> OPERATION_SHIFT)) {
            ...
            case LDY:
                ...
                break;
            case CPY: {
                    NES_Address diff = r_Y - m_bus.read(location);
                    f_C = !(diff & 0x100);
                    setZN(diff);
                }
                break;
            case CPX: {
                    NES_Address diff = r_X - m_bus.read(location);
                    f_C = !(diff & 0x100);
                    setZN(diff);
                }
                break;
